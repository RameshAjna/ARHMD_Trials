...
typedef PACKED_STRUCT {
    ...
    uint32_t reportInterval_uS;
    uint32_t batchInterval_uS;
    uint32_t acThreshold;  /**< @brief See SH-2 Reference Manual for details. */
    uint32_t stepThreshold;
} SetMotionReport_t;
...
typedef struct sh2_s {
    ...
    union {
        ...
        struct {
        	sh2_Motion_SensorConfig_t *pConfig;
                    sh2_SensorId_t sensorId;
                } getMotionSensorConfig;
        struct {
            const sh2_SensorConfig_t *pConfig;
            sh2_SensorId_t sensorId;
        } setSensorConfig;
        struct {
                    const sh2_Motion_SensorConfig_t *pConfig;
                    sh2_SensorId_t sensorId;
                } setMotionSensorConfig;
        struct {
            uint16_t frsType;
            uint32_t *pData;
            uint16_t *pWords;
...
// getSensorConfig Operation
static int getMotionSensorConfigStart(void);
static void getMotionSensorConfigRx(const uint8_t *payload, uint16_t len);
const sh2_Op_t getMotionSensorConfigOp = {
    .start = getMotionSensorConfigStart,
    .rx = getMotionSensorConfigRx,
};
...
// setSensorConfig Operation
static int setMotionSensorConfigStart(void);
static void setMotionSensorConfigTxDone(void);
const sh2_Op_t setMotionSensorConfigOp = {
    .start = setMotionSensorConfigStart,
    .txDone = setMotionSensorConfigTxDone,
};
// get FRS Operation
...
int sh2_initialize(sh2_EventCallback_t *eventCallback, void *resetCookie)
{
    ...
    shtp_listenChan("sensorhub", "control", sensorhubControlHdlr, NULL); //chan 2
    shtp_listenChan("sensorhub", "inputNormal", sensorhubInputNormalHdlr, NULL); //chan 3
    shtp_listenChan("sensorhub", "inputWake", sensorhubInputWakeHdlr, NULL); //chan 4
    shtp_listenChan("sensorhub", "inputGyroRv", sensorhubInputGyroRvHdlr, NULL); //chan 5
...
int sh2_setMotionSensorConfig(sh2_SensorId_t sensorId, const sh2_Motion_SensorConfig_t *pConfig)
{
    // Set up operation
    sh2.opData.setMotionSensorConfig.sensorId = sensorId;
    sh2.opData.setMotionSensorConfig.pConfig = pConfig;

    return opStart(&setMotionSensorConfigOp);
}
int sh2_getMotionSensorConfig(sh2_SensorId_t sensorId, sh2_Motion_SensorConfig_t *config)
{
    sh2.opData.getMotionSensorConfig.sensorId = sensorId;
    sh2.opData.getMotionSensorConfig.pConfig = config;

    return opStart(&getMotionSensorConfigOp);
}
...
void sensorhubAdvertHdlr(void *cookie, uint8_t tag, uint8_t len, uint8_t *val)
{
    switch (tag) {
...
static int getMotionSensorConfigStart(void)
{
    ...
    memset(&req, 0, sizeof(req));
    req.reportId = SENSORHUB_GET_FEATURE_REQ;
    req.featureReportId = sh2.opData.getMotionSensorConfig.sensorId;
    rc = shtp_send(sh2.controlChan, (uint8_t *)&req, sizeof(req));
    opTxDone();
...
static void getMotionSensorConfigRx(const uint8_t *payload, uint16_t len)
{
	GetMotionReport_t *resp = (GetMotionReport_t *)payload;
	sh2_Motion_SensorConfig_t *pConfig;

    // skip this if it isn't the response we're waiting for.
    if (resp->reportId != SENSORHUB_GET_FEATURE_RESP) return;
    if (resp->featureReportId != sh2.opData.getMotionSensorConfig.sensorId) return;

    // Copy out data
    pConfig = sh2.opData.getMotionSensorConfig.pConfig;
    ...
    pConfig->batchInterval_us = resp->batchInterval_uS;
//    pConfig->sensorSpecific = resp->sensorSpecific;
    pConfig->acThreshold = resp->acThreshold;  /**< @brief See SH-2 Reference Manual for details. */
    pConfig->stepThreshold = resp->stepThreshold;
...
static int setMotionSensorConfigStart(void)
{
	SetMotionReport_t req;
    uint8_t flags = 0;
    int rc;
    sh2_Motion_SensorConfig_t *pConfig = sh2.opData.getMotionSensorConfig.pConfig;
    ...
    req.reportInterval_uS = pConfig->reportInterval_us;
    req.batchInterval_uS = pConfig->batchInterval_us;
    printf("mot %d%d\r\n", pConfig->acThreshold, pConfig->stepThreshold);
    req.acThreshold = pConfig->acThreshold;
    req.stepThreshold = pConfig->stepThreshold;
