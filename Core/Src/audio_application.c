...
uint32_t PCM_Buffer[((AUDIO_IN_CHANNELS*AUDIO_IN_SAMPLING_FREQUENCY)/1000)  * N_MS] = {1};
...
* @retval None
*/
void Pause_Acquisition(void)
{
  if (CCA02M2_AUDIO_IN_Pause(CCA02M2_AUDIO_INSTANCE)!=BSP_ERROR_NONE)
  {
	printf("Unable to pause audio\r\n");
    Error_Handler();
  }
}
...
* @retval None
*/
void Resume_Acquisition(void)
{
  if (CCA02M2_AUDIO_IN_Resume(CCA02M2_AUDIO_INSTANCE)!=BSP_ERROR_NONE)
  {
	printf("Unable to resume audio\r\n");
    Error_Handler();
  }
}
